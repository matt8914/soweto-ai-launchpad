# Challenge Track Ideas

Choose a track, then start with one clear user problem. You can expand once the first demo works.

## Youth Employment and Skills Access

- CV and proof-of-work builder
- Learnership matcher
- Interview practice tracker
- Mentor-reviewed graduate profiles

## Township Business and Informal Economy

- Spaza stock helper
- Vendor order tracker
- Freelancer booking page
- Local creative portfolio marketplace

## Food Security and Urban Agriculture

- Food garden volunteer planner
- Surplus food redistribution board
- Community kitchen inventory tracker

## Community Safety and GBV Response

- Safe route reporting
- Referral directory
- Anonymous support request tracker

## Education and Learner Support

- Study group finder
- Homework support queue
- Career guidance assistant

## Health, Wellness and Mental Health

- Clinic navigation guide
- Counselling referral finder
- Anonymous wellness check-in

## Waste, Environment and Clean Communities

- Dumping report tracker
- Recycling pickup coordination
- Waste collector income board

## Civic Participation and Service Delivery

- Service issue report dashboard
- Community action planner
- Public information explainer
